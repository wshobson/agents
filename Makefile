...
.PHONY: help install install-full install-ocr deps check run run-full run-ocr clean
...
help:
	...
	@echo "  make install-full  Install all dependencies (including easyocr)"
	@echo "  make install-ocr   Install system OCR (tesseract) - requires sudo"
	@echo "  make deps          Show dependency status"
	...
	@echo "  ENGINE=<engine>    OCR engine: tesseract or easyocr"
...
install-full:
	$(PIP) install -r tools/requirements.txt easyocr
...
deps:
	...
	@$(PYTHON) -c "import yt_dlp; print('  ✓ yt-dlp', yt_dlp.version.__version__)" 2>/dev/null || echo "  ✗ yt-dlp"
	@$(PYTHON) -c "from youtube_transcript_api import YouTubeTranscriptApi; print('  ✓ youtube-transcript-api')" 2>/dev/null || echo "  ✗ youtube-transcript-api"
	...
	@$(PYTHON) -c "from colorthief import ColorThief; print('  ✓ colorthief')" 2>/dev/null || echo "  ○ colorthief (optional)"
	@$(PYTHON) -c "import easyocr; print('  ✓ easyocr')" 2>/dev/null || echo "  ○ easyocr (optional)"
