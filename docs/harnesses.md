...
| **GitHub Copilot** | supported | `.copilot/agents/` (agent profiles), `.copilot/skills/` (SKILL.md skills), `.copilot/commands/` (slash commands) |
...
| Slash commands | ✅ | converted to skills | ✅ | ✅ | TOML at `commands/` | ✅ via `.copilot/commands/` |
...
| Slash command (`commands/<x>.md`) | converted to skill | passed through | rewritten to `.opencode/commands/` | TOML at `commands/<plugin>/<x>.toml` | `.copilot/commands/<plugin>/<x>.md` |
...
.copilot/commands/                # generated by Copilot adapter
```

> Copilot discovers agents from `.copilot/agents/`, skills from `.copilot/skills/`, and slash commands from `.copilot/commands/` at the repo root. Run `make generate HARNESS=copilot` to create them locally.
...
> Copilot discovers agents from `.copilot/agents/`, skills from `.copilot/skills/`, and commands from `.copilot/commands/` at the repo level, and from `~/.copilot/agents/`, `~/.copilot/skills/`, and `~/.copilot/commands/` at the user level. The adapter emits to `.copilot/`; use `make install-copilot` for user-level discovery.
