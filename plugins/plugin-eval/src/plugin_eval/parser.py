...
_CODE_FENCE_PATTERN = re.compile(r"^\s*(?:>\s*)*(`{3,})(\w*)")
...
def _count_code_blocks(content: str) -> tuple[int, list[str]]:
    ...
    Every fenced block is delimited twice, so counting each ``` run would report
    double the number of blocks. Track whether a fence is currently open instead
    and count only the openers. A closing fence carries no language and has to be
    at least as long as the fence that opened the block.
    """
    count = 0
    languages: list[str] = []
    open_fence_length = 0
    ...
    for line in content.split("\n"):
        ...
        if open_fence_length:
            if not language and len(fence) >= open_fence_length:
                open_fence_length = 0
        else:
