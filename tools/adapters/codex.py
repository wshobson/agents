...
context file at the repo root (read directly by Codex/Cursor/OpenCode; Claude Code
reads it via `CLAUDE.md`, a symlink to `AGENTS.md`; redirected from Gemini via
`gemini-extension.json` / `.gemini/settings.json`). `emit_global` validates that file
fits Codex's 32 KiB cap and emits the marketplace registry.
...
class CodexAdapter(HarnessAdapter):
    ...
    def emit_global(self, plugins: list[PluginSource]) -> EmitResult:
        ...
        by Codex / Cursor / OpenCode; Claude Code reads it via `CLAUDE.md`, a symlink to
        `AGENTS.md`; redirected from Gemini via `.gemini/settings.json`). The adapter
        validates that the committed file fits Codex's constraints (32 KiB hard cap,
        ~150-line table-of-contents convention) and writes `.agents/plugins/marketplace.json`.
        ...
        if not agents_md.is_file():
            result.warnings.append(
                ...
                "context file for Codex/Cursor/OpenCode; Claude Code reads it via "
                "`CLAUDE.md`, a symlink to `AGENTS.md`; Gemini CLI redirects via "
                "`gemini-extension.json` / `.gemini/settings.json`."
            )
        else:
