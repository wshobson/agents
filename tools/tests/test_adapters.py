...
from tools.adapters.base import AgentSource, PluginSource, SkillSource, parse_frontmatter
from tools.adapters.codex import CodexAdapter, _POINTER_BYTES, _split_body_if_oversized
from tools.adapters.cursor import CursorAdapter
from tools.adapters.gemini import GeminiAdapter, _INLINE_BODY_THRESHOLD
...
class TestCodexAdapter:
    def test_emits_skill_with_namespaced_id(self, synthetic_plugin: PluginSource, output_root: Path):
    ...
    def test_strips_claude_only_skill_fields(self, tmp_path: Path, output_root: Path):
        ...
        plugin = PluginSource(name="demo", dir=plugin_dir, plugin_json={"name": "demo"}, skills=[skill])
    ...
    def test_emits_agent_toml_with_remapped_model(self, synthetic_plugin: PluginSource, output_root: Path):
    ...
    def test_agent_with_write_tools_gets_workspace_write(self, tmp_path: Path, output_root: Path):
        ...
        parsed = tomllib.loads((output_root / ".codex" / "agents" / "demo__writer.toml").read_text())
    ...
    def test_skill_body_splitting_overflow(self, tmp_path: Path, output_root: Path):
        ...
        overflow_path = output_root / ".codex" / "skills" / "demo__big" / "references" / "details.md"
    ...
    def test_agents_md_table_of_contents_shape(self, synthetic_plugin: PluginSource, output_root: Path):
        adapter = CodexAdapter(output_root=output_root)
        adapter.emit_plugin(synthetic_plugin)
        result = adapter.emit_global([synthetic_plugin])

        agents_md = output_root / "AGENTS.md"
        assert agents_md in result.written
        content = agents_md.read_text()
        # ≤ 150 lines
        assert content.count("\n") < 150
        # Must include the Map section
        assert "## Map" in content
        # Must reference docs/, not contain encyclopedic detail
        assert "docs/architecture.md" in content
        assert "docs/plugins.md" in content
...
class TestCursorAdapter:
    ...
    def test_emits_marketplace_with_owner_and_source(self, synthetic_plugin: PluginSource, output_root: Path):
        adapter = CursorAdapter(output_root=output_root)
        adapter.emit_plugin(synthetic_plugin)
        result = adapter.emit_global([synthetic_plugin])
    ...
    def test_string_author_normalized_to_dict(self, tmp_path: Path, output_root: Path):
        ...
        plugin = PluginSource(
            name="demo", dir=plugin_dir,
            plugin_json={"name": "demo", "version": "1.0.0", "author": "Jane Doe <jane@example.com>"},
        )
        result = CursorAdapter(output_root=output_root).emit_plugin(plugin)
        manifest = json.loads((output_root / ".cursor-plugin" / "plugins" / "demo.json").read_text())
...
class TestOpenCodeAdapter:
    ...
    def test_permission_block_denies_unlisted_tools(self, synthetic_plugin: PluginSource, output_root: Path):
        OpenCodeAdapter(output_root=output_root).emit_plugin(synthetic_plugin)
        agent_md = output_root / ".opencode" / "agents" / "demo__greeter.md"
...
class TestGeminiAdapter:
    def test_emits_native_skill_at_extension_root(self, synthetic_plugin: PluginSource, output_root: Path):
    ...
    def test_inline_command_when_body_is_small(self, synthetic_plugin: PluginSource, output_root: Path):
    ...
    def test_injection_when_body_is_large(self, tmp_path: Path, output_root: Path):
        ...
        cmd = _make_command(
            plugin_dir, "bigcmd", 'description: "Big"', big_body
        )
        plugin = PluginSource(name="demo", dir=plugin_dir, plugin_json={"name": "demo"}, commands=[cmd])
    ...
    def test_command_toml_parses_as_valid_toml(self, synthetic_plugin: PluginSource, output_root: Path):
        import tomllib
    ...
    def test_plugin_entry_command_at_top_level(self, synthetic_plugin: PluginSource, output_root: Path):
        GeminiAdapter(output_root=output_root).emit_plugin(synthetic_plugin)
